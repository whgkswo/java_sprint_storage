...
public class MyStorage {
    ...
    static void prod_remove(Scanner s){
        ...
        System.out.println("등록을 취소할 제품명을 입력해 주세요(1~5): ");
        String input;
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        for(int i = 0; i< amountInput.length(); i++){
            if(!refStr.contains(amountInput.charAt(i) +"")){
                isValid = false;
            }
        }
        if(isValid){
            int cnt = Integer.parseInt(amountInput);  // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        System.out.println("수량을 차감할 제품을 입력하세요");
        String input;
        String refStr = "0123456789";
        input = s.nextLine();
        int index = -1;
        ...
        if(counts[index] == 0){
            ...
            int maxNumber = counts[index];
            System.out.println(String.format("차감할 수량을 입력해 주세요 (1~%d)", maxNumber));
            int amount;
            boolean isValid = true;
            input = s.nextLine();
            ...
            if(isValid){
                ...
                if(amount > 0 && amount <= maxNumber){
                    counts[index] -= amount;
                    System.out.println(String.format("%s의 재고를 %d개 차감하였습니다. 현재 재고는 %d개입니다."
                            , products[index], amount, counts[index]));
                }else{
                    System.out.println(String.format("1~%d 사이 정수를 입력하세요", maxNumber));
                }
            }
